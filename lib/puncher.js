...
Puncher.prototype.start = function start(message, meta) {
  ...
  scope.stop    = null;
  scope.elapsed = {total: null, childs: null, self: null};
  scope.meta    = meta || {};
...
Puncher.prototype.stop = function stop(all) {
  ...
  if (scope) {
    ...
    scope.elapsed.childs  = get_elapsed_by_childs(scope);
    scope.elapsed.self    = scope.elapsed.total - scope.elapsed.childs;
