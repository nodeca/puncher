...
Puncher.MAX_NANOSECONDS = 1000000000;
...
function get_elapsed_by_childs(scope) {
  return scope.childs.reduce(function (memo, scope) {
    var result = [memo[0] + scope.elapsed.total[0], memo[1] + scope.elapsed.total[1]];
    if(result[1] >= Puncher.MAX_NANOSECONDS) {
        result[0]++;
        result[1] = Puncher.MAX_NANOSECONDS - result[1];
    }
    return result;
  }, [0,0]);
}
  ...
  if (scope) {
    ...
    if(scope.childs.length) {
        ...
        if( scope.elapsed.total[1] < child_elapsed[1]) {
            scope.elapsed.missed  = [scope.elapsed.total[0] - child_elapsed[0] - 1, Puncher.MAX_NANOSECONDS - Math.abs(scope.elapsed.total[1] - child_elapsed[1])];
        } else {
